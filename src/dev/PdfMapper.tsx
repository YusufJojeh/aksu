...
import { pdfCoordinatesByLocale, type FieldBox } from '../pdf/coordinates'
import { Button, Field, Input, Select } from '../components/ui'

pdfjs.GlobalWorkerOptions.workerSrc = new URL('pdfjs-dist/build/pdf.worker.min.mjs', import.meta.url).toString()

type TemplateLocale = keyof typeof pdfCoordinatesByLocale

function flatten(locale: TemplateLocale): Record<string, FieldBox> {
  const coordinates = pdfCoordinatesByLocale[locale]
...
export function PdfMapper() {
  ...
  const [template, setTemplate] = useState<TemplateLocale>('ar')
  const initial = useMemo(() => flatten(template), [template])
  const [coordinates, setCoordinates] = useState(initial)
  const [selected, setSelected] = useState('page1.patientName')
  const field = coordinates[selected]
  const pageNumber = selected.startsWith('page2.') ? 2 : 1
  const scale = 1.1

  useEffect(() => { setCoordinates(initial); setSelected('page1.patientName') }, [initial])
  useEffect(() => { void (async () => {
    const loadingTask = pdfjs.getDocument({ url: `./templates/${template}.pdf` })
