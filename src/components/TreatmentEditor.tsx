...
export function TreatmentEditor({ visit }: { visit: 'firstVisit' | 'secondVisit' }) {
  ...
  return <div className="space-y-2">
    ...
    {rows.map((row, index) => <div key={row.id} className="grid gap-2 rounded-lg border border-stone-200 bg-white p-3 lg:grid-cols-[32px_1.5fr_1fr_72px_96px_72px] lg:items-center lg:border-0 lg:border-b lg:bg-transparent lg:p-2">
      <Controller control={control} name={`${visit}.treatmentRows.${index}.enabled`} render={({ field }) => <input aria-label={t('fields.enabled')} type="checkbox" className="size-5 accent-gold" checked={field.value} onChange={field.onChange} />} />
      ...
      <div className="grid gap-2">
        ...
        <Controller control={control} name={`${visit}.treatmentRows.${index}.included`} render={({ field }) => <label className="flex items-center gap-2 text-xs text-stone-600"><input type="checkbox" className="accent-gold" checked={field.value} onChange={field.onChange} />{t('fields.included')}</label>} />
